...
au BufWinEnter * let w:m2=matchadd('Underlined', '\%>' . 160 . 'v.\+', -1)
...
let s:windows_CFlags = 'gcc\ -Wl,-enable-auto-import\ %\ -g\ -o\ %<.o'
let s:windows_CPPFlags = 'g++\ -Wl,-enable-auto-import\ %\ -g\ -o\ %<.o'
    ...
    if expand("%:e") == "c" || expand("%:e") == "cpp" || expand("%:e") == "cxx"
        ...
        if filereadable(Obj) && (getftime(Obj) >= getftime(Sou))
            ...
            if !executable(Exe) || (executable(Exe) && getftime(Exe) < getftime(Obj))
                if expand("%:e") == "c"
                    setlocal makeprg=gcc\ %\ -g\ -o\ %<.exe
		    "setlocal makeprg=gcc\ -o\ %<\ %<.o
                    echohl WarningMsg | echo " linking..."
                    silent make
                elseif expand("%:e") == "cpp" || expand("%:e") == "cxx"
                    setlocal makeprg=g++\ %\ -g\ -o\ %<.exe
    ...
    if expand("%:e") == "c" || expand("%:e") == "cpp" || expand("%:e") == "cxx"
        ...
        if executable(Exe) && getftime(Exe) >= getftime(Obj) && getftime(Obj) >= getftime(Sou)
            ...
            if g:iswindows
                ...
                if g:isGUI
                    exe ":!gnome-terminal -x bash -c './%<.exe; echo; echo 请按 Enter 键继续; read'"
                else
                    exe ":!clear; ./%<.exe"
                endif
            endif
...
function! ViewInBrowser(name)
    if expand("%:e") == "php" || expand("%:e") == "html" || expand("%:e") == "htm"
        ...
        if g:iswindows
            ...
            let Firefox = "D:/Program Files/firefox.exe"              "Firefox程序目录（可按实际更改）
